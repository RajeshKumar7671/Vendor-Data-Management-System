...
<table class="table">
    ...
    <tbody>
       ...
       <c:if test="${item.status eq 'Reject'}">
       <tr>
            <th scope="row">1</th>
			...
			<td>
               ...
               <a href="ApproveItem?itemId=<c:out value="${item.itemId}"/>" class="teal-text"><i class="glyphicon glyphicon-edit"></i></a>
               <a href="deleteRejectItem?itemId=<c:out value="${item.itemId}"/>" class="red-text"><i class="glyphicon glyphicon-trash"></i></a>
            </td>
          </c:if>				
	   </c:forEach>
    </tbody>
